# Reactive TS — live editor types (spike)

A minimal VS Code extension that gives `.rts` files **live TypeScript types**.
It contributes a tsserver plugin (`reactive-ts-plugin`) that, on every keystroke,
...
Open [`../examples/example.rts`](../examples/example.rts) and hover the bindings:
...
…or the Extensions view → **⋯** → **Install from VSIX…**. Then open any `.rts`
...
`build` / `build-and-install` delete any existing `reactive-ts-vscode-*.vsix` first, so
...
cd reactive-ts/ts-plugin && npm install

# 2. install build deps and produce the .vsix
cd ../vscode-extension && npm install && npm run package
```

This bundles the `reactive-ts-plugin` tsserver plugin self-contained (Volar inlined,
`typescript` left to the host) into `node_modules/reactive-ts-plugin/` and emits
`reactive-ts-vscode-<version>.vsix`.
...
1. Open the `reactive-ts/vscode-extension` folder.
2. Press **F5** ("Run Extension") to launch the **Extension Development Host**.
3. In that window, open `reactive-ts/examples/example.rts`.
4. Hover `x` / `w` — you should see `Observable<number>`.

> `npm run build` is what produces `node_modules/reactive-ts-plugin/` (the bundled
...
- The standalone Program resolves imports from disk relative to the file, so the
  `.rts` file must live inside this repo (where `node_modules/rxjs` is found).
- Only `+ - * /` on observables lift so far; no syntax highlighting for `.rts`
