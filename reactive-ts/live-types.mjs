...
//
// Simulates a hover: take a position in the .rts SOURCE, map it through to the
// generated TS, ask the TS language service for the type, and report diagnostics.
// Run: node reactive-ts/live-types.mjs
...
console.log(code);
console.log("=== LIVE HOVER (hover position in .rts source → inferred type) ===\n");
...
console.log("\n=== DIAGNOSTICS ===\n");
console.log(`  .rts source (raw, as plain TS): ${sourceDiagnostics.length} error(s)`);
