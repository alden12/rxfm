...
//
// The runtime is located by walking up from the .rts file's OWN directory, not
 ...
 *  imports this one) — a `.rts` importing another `.rts` re-runs it on the fly. */
...
//
// Cross-.rts imports: a `.rts` file importing another `.rts` must see the
...
// sibling `foo.rts` (when normal resolution finds nothing) and serve it as TS,
// transformed on the fly via the injected `transform`. (No cycle handling yet;
// .rts import graphs are shallow.)
export function createProgramFromText(
  ...
  host.getSourceFile = (name, languageVersion, ...rest) => {
    ...
    if (isRts(name)) return ts.createSourceFile(name, rtsCode(name), languageVersion, true, ts.ScriptKind.TS);
    return getSourceFile(name, languageVersion, ...rest);
  };
  host.fileExists = f => (f === fileName ? true : ts.sys.fileExists(f));
  const readFile = host.readFile.bind(host);
  host.readFile = f => (f === fileName ? text : isRts(f) ? rtsCode(f) : readFile(f));
