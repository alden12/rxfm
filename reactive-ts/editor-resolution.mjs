// Editor-side cross-.rts import resolution — headless reproduction + fix proof.
//
// In the editor, Volar serves each .rts file to tsserver as its *virtual* TS.
// A bare `import { seconds } from './producer'` must resolve to the sibling
// producer.rts (served as .ts) so the imported reactive value carries its real
...
// The import *target* keeps its .rts extension (so stock TS resolution genuinely
...
// Emulate Volar's decorated host: .rts files are served as their virtual TS.
function makeHost({ withFallback }) {
  ...
  if (withFallback) patchReactiveTsModuleResolution(ts, host);
  return host;
}
