...
7. **Runtime-import distribution (needed before tsrx merges).** The transform auto-emits the
   ...
   (a sibling of `tsrx/`) could resolve the canonical `tsrx/runtime.ts`. Decide a durable story —
   auto-inject these the way `render` is, resolve a stable bare specifier (e.g. `tsrx/runtime`), or
...
by the full-tsrx snake game (`getBoard(state.trail.coordinates, state.food)`).
...
- **Demonstrated by the full-tsrx snake game:** with C2/C3/C5 + the imperative analogues
...
- **Cross-`.tsrx` imports — transform side ✅, editor side ✅ (B1):**
  - **Transform side:** the transform's inference Program resolves a `./foo` import to a sibling
    `foo.tsrx`, transformed on the fly, so an imported reactive value carries its real type and
  ...
  - **Editor side (now fixed, ext 0.0.9):** the tsserver/Volar path type-checks the generated virtual
    ...
    that honours `resolveHiddenExtensions` and maps `./game` → `./game.tsrx` — when **at least one
    import literal in that file already ends in `.tsrx`**. A file with only bare specifiers falls
    through to stock TS resolution, which has no `.tsrx` knowledge → unresolved. So
  ...
  - **The fix:** the plugin (`ts-plugin/index.cjs`) layers a fallback over the host's
    ...
    unresolved relative import whose sibling `<spec>.tsrx` (or `<spec>/index.tsrx`) exists on disk is
    ...
    separate `.tsrx` files** — the snake is split into `game.tsrx` (engine) + `snake-game.tsrx` (view).
- **Pending (substantive roadmap):** declaration emit / incremental Program for a fast `tsc` build,
  `.tsrx` import cycle handling, then B2 (fixtures) and the rest of section B.
...
- **Related:** destructuring a stream into its fields (`const { board, gameStage } = game;`) should
  lift each field like the member-access form does (noted as a TODO in `game.tsrx`). Same root: the
...
- **Done:** the existing `observableRoots` single-root check (D3) was factored into a shared
  ...
  `d5.mjs`; `example.tsrx`'s `sum` comment updated to match.
