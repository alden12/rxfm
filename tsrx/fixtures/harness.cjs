// Shared fixture harness for the tsrx transform (CommonJS so Jest runs it with no
...
//
// The host mirrors the editor: it uses tsrx/tsconfig.json (same options as the
...
const TSRX_DIR = path.join(__dirname, '..');
...
// resolve exactly as they would for a real .tsrx file in that directory.
const genPathFor = baseDir => path.join(baseDir, '__fixture__.generated.ts');

// Parse tsrx/tsconfig.json once — the same compiler options the editor pins.
const PROJECT_OPTIONS = (() => {
  const configPath = path.join(TSRX_DIR, 'tsconfig.json');
  const { config } = ts.readConfigFile(configPath, ts.sys.readFile);
  return ts.parseJsonConfigFileContent(config, ts.sys, TSRX_DIR).options;
})();
 ...
 * @param {string} source raw .tsrx text
 * @param {string} [baseDir] dir the transform resolves the runtime relative to (default tsrx/)
 */
function run(source, baseDir = TSRX_DIR) {
