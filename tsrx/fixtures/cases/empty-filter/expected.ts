...
const labelled = render(score.pipe(map(score => score > 5)).pipe(switchMap(_cond => _cond ? of("hi") : EMPTY)));   // filter + map → maybe-empty
const safe = render(score.pipe(map(score => score > 10)).pipe(switchMap(_cond => _cond ? score : of(0))));         // total ternary → NOT maybe-empty
