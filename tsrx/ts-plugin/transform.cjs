...
function transformWithMappings(ts, sourceText, baseDir) {
    ...
    const options = (0, transform_program_cjs_1.getCompilerOptions)(ts);
    const program = (0, transform_program_cjs_1.createProgramFromText)(ts, fileName, sourceText, options, transformWithMappings);
    ...
    const { isObservableType, isObservableExpr, paramAcceptsObservable, observableValueType, emitsNever, noteStall, calleeEmitsNonCallable, typeIsCallable, returnTypeOf, memberTargetsValue, isElementValueType, isComponentMapCall, } = (0, transform_checker_cjs_1.createChecker)(ts, checker, sf, { observableBindings, aliasInfo, maybeEmptyBindings, nonCallableBindings, stalls });
    ...
    const observableRoots = (node) => {
        ...
        const walk = (n) => {
            ...
            if (ts.isIdentifier(n)) {
                ...
                if (ts.isQualifiedName(p) && p.right === n)
                    return;
                if ((ts.isPropertyAssignment(p) || ts.isShorthandPropertyAssignment(p)
                    || ts.isBindingElement(p) || ts.isParameter(p)) && p.name === n)
                    return;
    ...
    const combineMap = (sources, params, body) => {
        needed['rxjs/operators'].add('map');
        if (sources.length === 1)
            return [...sources[0], '.pipe(map(', params[0], ' => ', ...body, '))'];
    ...
    const expandAliases = (node, root) => {
        ...
        const walk = (n) => {
            if (ts.isIdentifier(n)) {
                ...
                if (ts.isQualifiedName(p) && p.right === n)
                    return;
                if ((ts.isPropertyAssignment(p) || ts.isBindingElement(p) || ts.isParameter(p)) && p.name === n)
                    return;
                if (ts.isShorthandPropertyAssignment(p))
                    return; // `{ color }` — leave (rare)
                if (aliasInfo.has(n.text)) {
                    occ.push({ id: n, alias: true });
                    return;
                }
                if (root && n.text === root.name && isObservableExpr(n)) {
                    occ.push({ id: n, alias: false });
                    return;
                }
                return;
            }
        ...
        for (const { id, alias } of occ) {
            if (id.getStart(sf) > cursor)
                pieces.push({ srcStart: cursor, srcEnd: id.getStart(sf) });
            if (alias) {
        ...
        if (cursor < node.getEnd())
            pieces.push({ srcStart: cursor, srcEnd: node.getEnd() });
        return pieces;
    };
    ...
    function transformExpression(node) {
        ...
        if (ts.isParenthesizedExpression(node)) {
            ...
            if (inner.lifted)
                return { pieces: inner.pieces, observable: true, lifted: true, callable: inner.callable };
            if (inner.rewritten)
                return { pieces: ['(', ...inner.pieces, ')'], observable: inner.observable, lifted: false, rewritten: true };
        ...
        if (ts.isTemplateExpression(node)) {
            ...
            if (spans.some(s => s.expr.observable)) {
                ...
                const subs = spans.map(s => {
                    if (!s.expr.observable)
                        return { inline: s.expr.pieces };
    ...
    const handleComponentMap = (node) => {
        ...
        if (objR.lifted)
            edits.push({ start: obj.getStart(sf), end: obj.getEnd(), pieces: objR.pieces });
        ...
        const destructures = itemParam && ts.isObjectBindingPattern(itemParam.name)
            && itemParam.name.elements.every(el => !el.dotDotDotToken && !el.initializer && ts.isIdentifier(el.name)
                && (!el.propertyName || ts.isIdentifier(el.propertyName)));
        if (destructures) {
            const synthetic = freshItemName(cb);
            const cbId = cbSeq++;
            edits.push({ start: itemParam.getStart(sf), end: itemParam.getEnd(), pieces: [synthetic] });
            observableBindings.add(synthetic);
            addedBindings.push(synthetic);
        ...
        for (let i = destructures ? 1 : 0; i < cb.parameters.length; i++) {
            ...
            if (!observableBindings.has(name)) {
                observableBindings.add(name);
                addedBindings.push(name);
            }
        }
        visit(cb.body);
        addedBindings.forEach(p => observableBindings.delete(p));
        for (const [key, prev] of addedAliases)
            prev ? aliasInfo.set(key, prev) : aliasInfo.delete(key);
    ...
    const collectHandlerCaptures = (fnNode) => {
        ...
        const walk = (n) => {
            if (ts.isIdentifier(n)) {
                const p = n.parent;
                if (ts.isPropertyAccessExpression(p) && p.name === n)
                    return; // property name, not a ref
                if (ts.isQualifiedName(p) && p.right === n)
                    return;
                if ((ts.isPropertyAssignment(p) || ts.isBindingElement(p) || ts.isParameter(p)) && p.name === n)
                    return;
                ...
                if (decl && within(decl, fnNode))
                    return; // local to handler, not a capture
                if (ts.isPropertyAccessExpression(p) && p.expression === n && !memberTargetsValue(n, p.name.text)) {
        ...
        walk(fnNode.body);
        for (const name of streamUsed)
            valueUsed.delete(name);
        return valueUsed;
    };
    ...
    const visit = (node) => {
        ...
        if (ts.isVariableDeclaration(node) && node.initializer) {
            ...
            if (r.observable && ts.isIdentifier(node.name)) {
                observableBindings.add(node.name.text);
                if (r.callable === false)
                    nonCallableBindings.add(node.name.text);
                if (r.emptyable)
                    maybeEmptyBindings.add(node.name.text);
            }
            if (r.lifted) {
        ...
        if (ts.isCallExpression(node)) {
            ...
            node.arguments.forEach((arg, i) => {
                ...
                if (isComponentMapCall(arg)) {
                    handleComponentMap(arg);
                    return;
                }
                ...
                if ((ts.isArrowFunction(arg) || ts.isFunctionExpression(arg)) && paramAcceptsObservable(node, i)) {
                    ...
                    if (handler) {
                        edits.push({ start: arg.getStart(sf), end: arg.getEnd(), pieces: handler });
                        return;
                    }
                }
    ...
    const { code, segments } = (0, transform_emit_cjs_1.assembleOutput)({
