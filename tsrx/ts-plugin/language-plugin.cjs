...
exports.getTransformResult = void 0;
exports.createTsrxLanguagePlugin = createTsrxLanguagePlugin;
...
function createTsrxLanguagePlugin(ts) {
    ...
    const compile = (filePath, text) => {
        ...
        if (hit && hit.text === text)
            return hit.result;
        const result = (0, transform_cjs_1.transformWithMappings)(ts, text, path.dirname(filePath));
    ...
    const makeVirtualCode = (filePath, snapshot) => {
        ...
        return {
            ...
            mappings: (0, transform_cjs_1.segmentsToVolarMappings)(segments),
            embeddedCodes: [],
        };
    };
    return {
        getLanguageId(uri) {
            if (uriPath(uri).endsWith('.tsrx'))
                return 'tsrx';
            return undefined;
        },
