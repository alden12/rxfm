...
import { createTsrxLanguagePlugin, getTransformResult } from './language-plugin.cjs';
...
function tsrxTransformFor(ts: Ts, info: any, fileName: string) {
...
function resolveTsrxSibling(ts: Ts, containingFile: string, specifier: string) {
  if (!specifier.startsWith('.')) return undefined;
  const base = path.resolve(path.dirname(containingFile), specifier);
  const candidate = [base + '.tsrx', path.join(base, 'index.tsrx')].find((f: string) => ts.sys.fileExists(f));
...
function patchTsrxModuleResolution(ts: Ts, languageServiceHost: any) {
  ...
  if (resolveNames) {
    languageServiceHost.resolveModuleNames = (names: string[], containingFile: string, ...rest: any[]) => {
      ...
      return resolved.map((module: any, i: number) => module || resolveTsrxSibling(ts, containingFile, names[i]));
    };
  }
}
