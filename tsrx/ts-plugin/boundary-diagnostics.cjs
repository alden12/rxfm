...
exports.BOUNDARY_CODES = void 0;
exports.rewriteBoundaryDiagnostic = rewriteBoundaryDiagnostic;
...
exports.BOUNDARY_CODES = new Set([PROPERTY, NOT_CALLABLE, NOT_CONSTRUCTABLE, NOT_INDEXABLE]);
...
function teachingHeadline(flat, code, kind, inner) {
    ...
    if (code === PROPERTY) {
        ...
        return (`'${name}' isn't available here — this is ${label} (${type}), not a ${inner}, ` +
    ...
    if (code === NOT_CALLABLE) {
        return (`You're calling ${label} (${type}) directly, but it's a stream of future values, ` +
            `not a function to invoke now. Map over it instead, e.g. ` +
            '`value.pipe(map(v => v(/* … */)))`.');
    }
    if (code === NOT_CONSTRUCTABLE) {
        return (`You're using \`new\` on ${label} (${type}), but it's a stream, not a constructor. ` +
            'Construct inside a map instead, e.g. `value.pipe(map(v => new v(/* … */)))`.');
    }
    if (code === NOT_INDEXABLE) {
        return (`You're indexing ${label} (${type}) directly, but it has no value to index ` +
...
function rewriteBoundaryDiagnostic(ts, diagnostic) {
    if (!diagnostic || !exports.BOUNDARY_CODES.has(diagnostic.code))
        return null;
